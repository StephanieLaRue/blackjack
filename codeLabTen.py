...
from myclass import DoubleDown
...
def starting_money():
    ...
    while True:
        try:
            ...
            print("\nMoney:", locale.currency(round(user_money, 2)))
...
def playing(hand, deck, bet):
    while True:
        try:
            ...
            if hit_stand != "d" and hit_stand != "h" and hit_stand != "s":
                print("\nInvalid entry. Hit, Stand, or Double Down? (h/s/d)")
            else:
                if hit_stand == "h":
                    ...
                    if isBusted(player_points):
                        break

                elif hit_stand == "s":
                    break
                elif hit_stand == "d":
                    money = float(get_money())
                    bet_amount = bet * 2
                    if money < bet_amount:
...
def get_results(result, bet, money):
    ...
    if result == "b":
        ...
        money -= bet
        print("You lost!")
    print("Money:", locale.currency(round(money, 2)))
    print()
...
def display_time(started, stopped):
    ...
    print("Elapsed time:", time(hours, minutes, seconds))
...
def main():
    ...
    while True:
        ...
        if bet_amount < 5:
            ...
            outcome = play_hand(deck, bet_amount)
            get_results(outcome, bet_amount, player_money)
            if not continue_game():
                break
    print("\nCome again soon!")
    stop_time = get_time()
    print("\nStop time:   ", stop_time.strftime("%I:%M:%S %p"))
    display_time(start_time, stop_time)
    print("\nGoodbye, friend!")
